...
uint32_t endian(uint8_t *s)
{
	return s[0] << 24 | s[1] << 16 | s[2] << 8 | s[3] ;
}

uint32_t endian2(uint8_t *s)
{
...
#ifndef BIG_ENDIAN
#if (WORD_SIZE == 4)
#define ENDIAN(S) (S[0]<<24 | S[1]<<16 | S[2]<<8 | S[3])
#elif (WORD_SIZE == 2)
#define ENDIAN(S) (S[0]<<8 | S[1])
#elif (WORD_SIZE == 1)
#define ENDIAN(S) (S[0])
#endif
#else
#define ENDIAN(S) * (DTYPE *) (S)
#endif


void bignum_init2(struct bn *n, uint8_t *str, uint32_t nbytes)
{
	require(n, "n is null");
	...
	int i = nbytes - (WORD_SIZE); /* index into string */
	...
	while (i >= 0)
	{
		...
		n->array[j] = tmp;
		i -= (WORD_SIZE); /* step WORD_SIZE hex-byte(s) back in the string. */
...
DTYPE_TMP bignum_to_int(struct bn *n)
{
	...
	/* Endianness issue if machine is not little-endian? */
#if (WORD_SIZE == 1) // DTYPE_TMP = uint32_t;
	ret += n->array[0];
	ret += n->array[1] << 8;
	ret += n->array[2] << 16;
	ret += n->array[3] << 24;
#elif (WORD_SIZE == 2) // DTYPE_TMP = uint32_t;
	ret += n->array[0];
	ret += n->array[1] << 16;
#elif (WORD_SIZE == 4) // DTYPE_TMP = uint64_t;
	ret += n->array[0];
#endif
